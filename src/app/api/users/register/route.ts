...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      return NextResponse.json({ error: "Validation failed", details: vErr.errors }, { status: 400 });
    }
    ...
    const user = await User.create({ name, email, passwordHash, provider: "credentials", role: "client" });

    // send welcome email (best-effort)
    try {
      const html = `<p>Hi ${name},</p><p>Welcome to ${process.env.NEXT_PUBLIC_APP_NAME || "our store"}! We're glad you joined.</p>`;
