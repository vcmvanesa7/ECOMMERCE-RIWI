...
export default function ProductPage({ params }: { params: { id: string } }) {
  ...
  const variantStock = useMemo(() => {
    if (!product) return 0; // Prevents TS error
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Encontrar primera variante con stock
      const validVariant = p.variants?.find((v) => v.stock > 0);

      // Seleccionar color / size adecuados
  ...
  return (
    <div className="min-h-screen pt-24 pb-28 bg-white text-neutral-900">
      {/* BREADCRUMB */}
      ...
      <div className="max-w-[1400px] mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div>
          ...
          <p className="text-sm mt-1 text-neutral-500">
            Brand: <span className="text-neutral-800">{product.brand}</span>
          </p>
        ...
        <div className="max-w-[1400px] mx-auto px-6 mt-20">
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm text-neutral-500">(221 reviews)</p>
          </div>

          <p className="mt-4 text-neutral-600">
            “Amazing quality. Colors are vibrant and the fit is perfect. Exactly
            what I expected from KOI Streetwear.”
          </p>
        </div>
      </div>
    </div>
